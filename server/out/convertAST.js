        ...
        switch (oldblock.kind) {
            case oldast.ASTKinds.Block_1: { //multiple includes
                ...
                for (const oldinclude of oldblock.include) {
                    let ni = new newast.Include(oldinclude.name.value, newblock, oldinclude.posS, oldinclude.posE);
            ...
            case oldast.ASTKinds.Block_2: { //single include
                let newblock = new newast.Block_Include(root, oldblock.pos);
                root.blocks.push(newblock);
                let ni = new newast.Include(oldblock.include.name.value, newblock, oldblock.include.posS, oldblock.include.posE);
                newblock.includes.push(ni);
                break;
            }
            case oldast.ASTKinds.Block_3: { //type
                ...
                for (const oldtype of oldblock.type) {
                    let newtype = new newast.Type(oldtype.typeName.value, newblock, oldtype.posS, oldtype.posE);
            ...
            case oldast.ASTKinds.Block_7: { //multiple processes
                let newblock = new newast.Block_Process(false, root, oldblock.pos);
                root.blocks.push(newblock);
            ...
            case oldast.ASTKinds.Block_8: { //single process
                let newblock = new newast.Block_Process(true, root, oldblock.pos);
                root.blocks.push(newblock);
...
function flattenTEProduct(typeExp) {
    var children = [];
    children.push(typeExp.left);
    if (typeExp.right.kind == newast.ASTKinds.TE_Product) {
        children.push(...flattenTEProduct(typeExp.right));
    }
    else {
...
function flattenDETuple(dataExp) {
    var children = [];
    children.push(dataExp.left);
    if (dataExp.right.kind == newast.ASTKinds.DE_Tuple) {
        children.push(...flattenDETuple(dataExp.right));
    }
    else {
...
function convertConVar(node, parent, isVar) {
    ...
    switch (node.kind) {
        ...
        case oldast.ASTKinds.ConVar_2: { //singular convar
            if (isVar) {
                convar = [new newast.Variable(node.name.value, parent, node.posS, node.posE)];
            }
            else {
                convar = [new newast.Constant(node.name.value, parent, node.posS, node.posE)];
            }
            break;
        }
    }
...
function convertLRDataExp(node, parent) {
    ...
    switch (node.kind) {
        ...
        case oldast.ASTKinds.DE1_1:
            newnode = new newast.DE_Function_Infix(5, parent, node.posS, node.posE);
            newnode.function = new newast.Function_Infix("->");
        ...
        case oldast.ASTKinds.DE1_3:
            newnode = new newast.DE_Function_Infix(4, parent, node.posS, node.posE);
            newnode.function = new newast.Function_Infix("&");
            insertLRNodeDE(newnode);
            newnode.right = convertDataExp(node.dataExp, newnode);
            break;
        ...
        case oldast.ASTKinds.DE1_5:
            newnode = new newast.DE_Function_Infix(3, parent, node.posS, node.posE);
            newnode.function = new newast.Function_Infix("=");
            insertLRNodeDE(newnode);
            newnode.right = convertDataExp(node.dataExp, newnode);
            break;
        ...
        case oldast.ASTKinds.DE1_11:
            newnode = new newast.DE_Function_Infix(2, parent, node.posS, node.posE);
            newnode.function = new newast.Function_Infix(":");
            insertLRNodeDE(newnode);
            newnode.right = convertDataExp(node.dataExp, newnode);
            break;
        ...
        case oldast.ASTKinds.DE1_12:
            newnode = new newast.DE_Function_Infix(1, parent, node.posS, node.posE);
            newnode.function = new newast.Function_Infix(node.func.value);
            insertLRNodeDE(newnode);
            newnode.right = convertDataExp(node.dataExp, newnode);
            break;
        //DE1_13 is DE_Tuple
